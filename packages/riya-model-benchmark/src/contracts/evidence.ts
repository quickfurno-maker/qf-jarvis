...
/** True iff `value` is a canonical UTC instant that also parses to a real calendar time. */
...
/**
 * True iff `evidence` still hashes to the digest it carries.
 *
 * Cheap, total, and it never throws — a caller checking a list of artifacts wants a boolean per item,
 * not an exception that stops the loop at the first bad one.
 */
export function riyaBenchmarkEvidenceIntegrityHolds(evidence: RiyaBenchmarkEvidenceV1): boolean {
  const { evidenceDigest, ...rest } = evidence;
  return sha256OfCanonical(digestBody(rest)) === evidenceDigest;
}
