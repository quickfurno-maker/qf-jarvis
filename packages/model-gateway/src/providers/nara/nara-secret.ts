/**
 * The Nara API-key holder (JF-2A, ADR-0146).
 *
 ...
 * or a report. Only the transport boundary reads {@link NaraApiKey.authorizationHeaderValue}; the value
 * itself is never otherwise exposed and there is NO environment-variable access anywhere in this package.
 *
 * A separate class from the Groq holder on purpose. They are structurally identical and that is not a
 * reason to share one: a single holder used by two providers is a value that can be handed to the wrong
 * transport, and the type system would not object. Two types make that a compile error.
 */
const REDACTED = '[REDACTED_NARA_API_KEY]';

export class NaraApiKey {
 ...
 */
export function createNaraApiKey(value: string): NaraApiKey {
  if (typeof value !== 'string' || value.trim().length < 1 || value.length > 512) {
    throw new Error('A Nara API key must be a non-empty bounded string (injected at composition).');
  }
  return new NaraApiKey(value);
}
