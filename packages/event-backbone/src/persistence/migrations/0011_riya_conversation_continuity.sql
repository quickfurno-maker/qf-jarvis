...
--
-- It is NOT a transcript -- but the precise claim matters more than a sweeping one. What is stored is
-- no transcript, no raw message history, no recent-turn history, no rolling/conversation/memory
-- summary, no raw model reply or draft, no contact data, no consent or business authority, and no
-- independent free-text blob. What IS stored, beyond that, is the P2A/ADR-0067 structured
-- NeedDiscovery snapshot -- and that snapshot legitimately carries BOUNDED textual notes:
-- `scopeSummary` (<=500 chars), `budgetNote` and `timelineNote` (<=120 each). So this table is not
-- "content-free", and saying so would be untrue; it is content-MINIMISED to a governed shape. A
-- second free-text column would be a transcript with a friendlier name, and the constructor refuses
-- one.
--
...
COMMENT ON TABLE qf_jarvis.riya_conversation_continuity IS
  ...
  'profile, NOT a CRM record, NOT training data and NOT business truth. Content-MINIMISED, not '
  'content-free: it stores no transcript, raw message history, recent-turn history, rolling summary, '
  'raw model reply or draft, channel, user id, phone, email, name, browser or session token, provider '
  'message id, consent, canSubmit, lead, vendor, city authority, price, package or independent '
  'free-text blob -- but the ADR-0067 discovery snapshot it does store may carry bounded notes such '
  'as scopeSummary, budgetNote and timelineNote. A durable row is born at continuity_revision 0 and '
  'every accepted replacement advances it by exactly one. summary_confirmed is a CONVERSATIONAL fact '
  'and is never consent. The authoritative validator is createRiyaConversationContinuityState; these '
  'constraints validate evidence and are not a second decision engine.';

-- ---------------------------------------------------------------------------
-- 2. Revision enforcement -- born at zero, and every replacement advances by exactly one
-- ---------------------------------------------------------------------------
--
-- `continuityRevision` is declared a MONOTONIC compare-and-set counter by RWC-P2A, and the sibling
-- domain fixes what that phrase means here: `agent-runtime`'s orchestration contract records that a
-- conversation revision's "domain is fixed by the durable schema that owns it", and 0008 implements
-- exactly that for the control counter. This table is the durable schema that owns THIS counter, so
-- the property lives here rather than in an adapter promise.
--
-- WHY THIS IS NOT OPTIONAL. Without it, a caller may compare-and-set `expectedRevision = 5` while
-- supplying a next state that is also revision 5. The row lock releases, the stored revision is
-- unchanged, and a SECOND writer still holding 5 then matches the predicate and succeeds too -- both
-- writers are told UPDATED and the first one's state is silently destroyed. A next revision LOWER
-- than the current one is worse: the counter runs backwards. Requiring exactly OLD + 1 makes both
-- unrepresentable from ANY writer -- this adapter, a future second adapter, a migration, or a console
-- session -- rather than only from the one that promises to behave.
--
-- It is NOT a reducer. It decides no phase transition, merges no discovery, resolves no provenance,
-- computes nothing, and never rewrites NEW into compliance. It validates identity and the counter,
-- and refuses. RWC-P4 still owns every decision about what the next state CONTAINS.
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    -- A durable continuity row is BORN at zero. `createInitialIfAbsent` is initial persistence, not a
    -- back door for arriving at an arbitrary revision without passing through the compare-and-set
    -- path that every later revision must be reached by.
  ...
  -- would be a different conversation wearing an existing row's history and revision.
  ...
  -- Overflow before it happens. At the safe-integer ceiling there is no representable next value, and
  -- an adapter could not compare OLD + 1 to the caller's revision anyway.
  ...
  -- EXACTLY one. Not "greater than": a gap would leave the counter an arbitrary increasing tag whose
  -- jumps nobody can account for, and the adapter's own check is `expectedRevision + 1`.
...
COMMENT ON FUNCTION qf_jarvis.riya_conversation_continuity_guard() IS
  'Requires a new row to be created at continuity_revision 0, refuses an identity change, refuses '
  'advancing an exhausted revision, and requires every UPDATE to advance continuity_revision by '
  'exactly one. A storage concurrency invariant, not a phase reducer: it decides no transition, '
  'merges no discovery or provenance, and never rewrites NEW.';
