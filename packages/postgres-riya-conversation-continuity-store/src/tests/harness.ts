 ...
 * concurrency are the entire point of RWC-P2B: a suite that quietly reported success without a
 ...
 * Migration 0011 is LOCAL/CI only; the managed QF-Jarvis database is untouched by this phase, and
...
export function requireTestDatabaseUrl(): string {
  ...
  if (url === undefined || url.length === 0) {
    throw new Error(
      'DATABASE_URL is required for the RWC-P2B durable continuity store tests. They prove restart ' +
        'durability, single-winner create arbitration and compare-and-set under real concurrency; ' +
