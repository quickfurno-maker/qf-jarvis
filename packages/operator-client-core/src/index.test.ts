...
const snapshot = {
  ...
  mode: 'READ_ONLY',
  source: { kind: 'REPOSITORY_BASELINE', freshness: 'BUILD_DECLARATION', liveOperationalData: false },
  authority: {
  ...
  system: [], capabilities: [], agents: [], roadmap: [],
  sections: {
    headlineMetrics:{availability:'STATIC_BASELINE',reason:'r',expectedSource:'s',items:[]},
    ...
    approvalBreakdown:{availability:'NOT_CONNECTED',reason:'r',expectedSource:'s',items:[]},
    conversationControl:{availability:'NOT_CONNECTED',reason:'r',expectedSource:'s',items:[]},
    conversationActivity:{availability:'NOT_CONNECTED',reason:'r',expectedSource:'s',id:'conversation-activity',label:'Conversation activity',points:[]},
    modelLatency:{availability:'NOT_CONNECTED',reason:'r',expectedSource:'s',id:'model-latency',label:'Model latency',points:[]},
    ...
    aarohiAcquisitionReadiness:{availability:'STATIC_BASELINE',reason:'r',expectedSource:'s',items:[]},
    ...
    businessAnalytics:{availability:'NOT_CONNECTED',reason:'r',expectedSource:'s',items:[]},
    coreAutomationExecution:{availability:'NOT_CONNECTED',reason:'r',expectedSource:'s',items:[]},
  },
} as const;
...
describe('operator client core', () => {
  it('parses the shared v2 snapshot without platform code', async () => {
    const client = createOperatorClient({
      platform: 'ANDROID',
      transport: { request: async () => ({ status: 200, body: snapshot }) },
    });
