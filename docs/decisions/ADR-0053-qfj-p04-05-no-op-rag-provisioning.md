...
> **Currency note (JF-3, 2026-09-10).** [ADR-0148](./ADR-0148-jf3-governed-exact-rag-activation-for-production-v1.md) supersedes the no-op restriction above **for exactly one backend kind**: a third mode `ACTIVE` and one backend `GOVERNED_EXACT`, which delegates to the QFJ-P04.03 knowledge authority under an exact profile-to-backend `knowledgeRevision` binding. Everything else on this page still holds and is still enforced by spec: `DISABLED` remains the default, `PROVISIONED_NO_OP` behaviour is preserved byte-for-byte, there is still no `ENABLED` mode and no `enabled=true`, the `FUTURE_*` vector placeholders are still refused at runtime, and there are still **no embeddings, no vector database, no semantic search, no similarity ranking, no free-text query, no chunking, no indexing, no external service, no network and no migration**. This ADR remains historical truth for the slice it governs.
