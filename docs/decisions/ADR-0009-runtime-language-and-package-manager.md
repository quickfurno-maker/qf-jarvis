...
- **Strict by configuration.** `strict-peer-dependencies`, `engine-strict`, exact saves, and a release cooldown are all first-class ([.npmrc](../../.npmrc), [pnpm-workspace.yaml](../../pnpm-workspace.yaml)).
...
- **Dependencies:** exact versions in `package.json` — `"typescript": "6.0.3"`, never `"^6.0.3"`. Enforced by `save-exact=true`.
...
- **Node** is pinned in four places that must agree: `.nvmrc`, `.node-version`, `engines.node` (`>=24.18.0 <25.0.0`, enforced by `engine-strict=true`), and the CI workflow.
...
**Release cooldown.** A package version must have been public for at least 24 hours before this repository will resolve it (`minimumReleaseAge`, with `minimumReleaseAgeStrict`). The realistic npm supply-chain attack is a compromised maintainer account publishing a malicious patch, typically detected and unpublished within hours. Because we pin exactly and upgrade deliberately, we are never racing to a release, so this costs us nothing and removes an entire attack window.
...
| **`engine-strict` blocks a developer** on the wrong Node version                             | That is the intent: fail at install with a clear message rather than at runtime with a strange one. Corepack and `.nvmrc` make the fix a single command                                                                                                                                                                                                                                                |
| **The four Node pins drift apart** (`.nvmrc`, `.node-version`, `engines.node`, CI)           | CI runs the pinned version and `engine-strict` enforces the constraint, so a drift fails the build rather than passing quietly                                                                                                                                                                                                                                                                         |
