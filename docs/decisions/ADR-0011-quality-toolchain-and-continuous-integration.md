...
The full verified matrix is in [supported-toolchain.md](../engineering/supported-toolchain.md). The install runs with `strict-peer-dependencies=true` and produces **no peer warnings**, and **no package in the dependency tree requires an install or build script** — so `onlyBuiltDependencies` is empty, and no package has been granted install-time code execution.
