...
Groq returns model output only; M4 makes a draft; **QuickFurno Core is the final authority.** No send/deliver/execute/QuickFurno Core Automation. No live call/key/activation; no DB/migration 0008; no RAG; no dashboard/deployment. Kimi excluded. The minimum Conversation Operations Center remains mandatory before a pilot and is not implemented here.
