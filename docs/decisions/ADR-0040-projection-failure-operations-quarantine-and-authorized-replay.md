...
**Recovery invariants (detailed in the design document).** (a) **Unknown classification:** unknown/missing-code/unreadable/hostile thrown values must not auto-become deterministic; a distinct `UNKNOWN_UNCLASSIFIED_FAILURE` fails closed (checkpoint unchanged, no deterministic exhaustion, sanitized code, escalate). The current runner's classification of an **absent SQLSTATE** as deterministic is **current behaviour requiring correction in QFJ-P03.07B**. (b) **Reconciliation biconditional:** a blocked checkpoint ↔ exactly one matching unresolved active failure, with fail-closed reconciliation for all seven divergences. (c) **Replay crash / unknown commit:** a durable `REPLAYING` state needs an execution owner, bounded lease, expiration, takeover and reconciliation rules; the failure rests at `REPLAY_AUTHORIZED` with an append-only `STARTED` attempt; an unknown commit never triggers a blind duplicate replay and reconciles first. (d) **Atomic completion:** success commits read-model mutation + checkpoint advance + attempt success + failure resolution + authorization consumption + append-only audit in **one transaction owned by the replay runner (QFJ-P03.07F)**; if ever not one transaction, a durable outbox with fail-closed reconciliation is required and best-effort follow-up is prohibited. Full specification: [projection-failure-operations.md](../architecture/projection-failure-operations.md).
