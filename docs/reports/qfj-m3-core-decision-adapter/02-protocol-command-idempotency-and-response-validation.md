...
`buildCoreCommand({ request, protocol, correlationId, createdAt })` returns a **frozen** `CoreCommand` (its nested protocol, structured intent, and citations are frozen too). It binds `commandId = ${conversationId}-${proposalId}-r${expectedRevision}`, the idempotency key, the correlation id, and the exact proposal/conversation/revision/actor/party/kind/policy identity. It is **rejected** (throws `invalid-command`) when the protocol is invalid, when `createdAt` is not a canonical UTC ISO-8601 instant, or when `correlationId` is not a bounded identifier. A **reply body is present only for a `REPLY`** proposal; for any other kind it is `undefined`. The command carries **no** chain-of-thought, raw provider body/header, SDK object, secret, callback, QuickFurno Core Automation command, delivery mutation, or DB handle — proven by scanning the serialized wire form for `reasoning`, `chainOfThought`, `sk-`, `apiKey`, `__proto__`, `rawResponse`.
