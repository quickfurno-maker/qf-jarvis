...
Riya client-only; Anisha vendor-only; Jarvis coordinator; **QuickFurno Core** is the final business authority; QuickFurno Core Automation is transport/execution only (absent here); models/providers authorize and execute nothing; Kimi excluded; RAG disabled. The minimum Conversation Operations Center remains mandatory before a pilot (not implemented here). Groq returns model output only; M4 makes a draft. **No real key, no live call, no activation/rollout, no Core/WhatsApp/QuickFurno Core Automation/send, no persistence/DB/migration 0008, no dashboard/deployment.** The `@qf-jarvis/event-backbone` root API remains **39**; migrations 0001–0007 are byte-exact with no 0008.
