...
A **preview-first agent scaffold with a best-effort QuickFurno Core Automation webhook bridge.** It is genuinely careful about not causing side effects — and it is genuinely **not a durable event source.** Both halves of that sentence matter.
...
| **A non-2xx from QuickFurno Core Automation is reported as OK** | `sendEventToQuickFurno Core Automation` returns `createSafeQuickFurno Core AutomationWebhookFailureResult(...)` → `ok: true, status: "mocked"` — `lib/aos/tools/coreAutomationTool.ts:139-142,211-215` | **A 500 from QuickFurno Core Automation looks exactly like "QuickFurno Core Automation is disabled."** The event is gone and nothing is red |
| **Timeout mismatch**                                            | Emitter gives up at `AOS_EMIT_TIMEOUT_MS = 3_000` (`emitLeadCreatedEvent.ts:22`); the webhook runs to `QuickFurno Core Automation_WEBHOOK_TIMEOUT_MS = 8_000` (`coreAutomationTool.ts:18`)             | The emitter logs "timed out" **while the request may still deliver** — a reported failure that succeeded                                    |
| **No queue**                                                    | `queueEventForQuickFurno Core Automation` is a misnomer; `databasePersisted: false` — `lib/aos/sync/QuickFurno Core AutomationSyncService.ts:184`                                                      | **A failed webhook is a permanently lost event**                                                                                            |
...
**Lock 1 — environment** (`lib/aos/config/featureFlags.ts:28-29`, overridable via env at `coreAutomationTool.ts:198-204`):
`QuickFurno Core Automation_ENABLED` **and** `QuickFurno Core Automation_OUTBOUND_WEBHOOK_ENABLED`, **both** required.

**Lock 2 — database** (`lib/aos/runtime/aosRuntimeSettings.ts:189-209`):
the `aos_coreAutomation_master_router` row in `aos_runtime_settings` must be `enabled = true` **and** `mode = 'preview'`. Seeded off. Superadmin-only. `production_locked` is refused.

```ts
const runtimeReady = runtime.enabled && runtime.mode === 'preview';
const shouldCallQuickFurno Core Automation = envLock.bothEnabled && runtimeReady;
```

Both locks open ⇒ a **real `fetch()` POST** to `QuickFurno Core Automation_WEBHOOK_URL`. This is the only real outbound call in the AOS path, and the design is sound: two independent locks, one of which is not deployable by a code change alone.

> **The hardcoded `false` values are not the safety property.** `QuickFurno Core Automation_ENABLED` and `QuickFurno Core Automation_OUTBOUND_WEBHOOK_ENABLED` are **environment-overridable**, so the constants in `featureFlags.ts` describe the default, not the guarantee. `WHATSAPP_SENDING_ENABLED`, `CREDIT_DEDUCTION_ENABLED` and `AUTO_ASSIGNMENT_ENABLED` have **no env path** and require a code change — those three are genuinely hard-off.

### The side-effect report is cosmetic

`QuickFurnoSafeSideEffectReport` (`lib/aos/events/coreAutomationEventTypes.ts:27-35`) has seven booleans. **Only `QuickFurno Core AutomationWebhookCalled` is ever set true.** The other six (`whatsappSent`, `creditsDeducted`, `leadAutoAssigned`, …) are hardcoded `false` and **nothing reads them to decide anything.** It is a reporting struct that looks like a gate, which is the more dangerous of the two things to be.

---

## 4. Masking — real, partial, and not what the comment says

- The `lead` object carrying **raw name and phone** is built in `emitLeadCreatedEvent.ts:58-66`, but `normalizeSafeAgentEventPayload` (`safeAgentEventPipeline.ts:207-223`) **drops it entirely** — only `eventType`, `leadId`, `source`, `timestamp`, `safeData` are forwarded. **Raw PII never reaches QuickFurno Core Automation, by field-dropping rather than by masking.**
...
- Real masking (`coreAutomationTool.ts:243-248,344-350`) applies only to `data`/`metadata`: secret-like keys → `[redacted]`, phone-like keys and digit runs → masked.
...
**21 QuickFurno Core Automation event names** are defined (`lib/aos/events/coreAutomationEventTypes.ts:1-23`) — the real surface. **Only 12 are actually emittable**: `normalizeSafeEventType` (`safeAgentEventPipeline.ts:225-240`) **silently coerces any other name to `aos.failure`**.
...
| #   | Current QuickFurno Core Automation event | Emittable?    | Disposition     | Canonical Jarvis target                                              | Note                                                                                                                  |
| --- | ---------------------------------------- | ------------- | --------------- | -------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
...
- **`coreAutomationPreviewWorkflowMap.ts`** — four display-only names (`lead.quality_preview`, `vendor.match_preview`, `client.followup_preview`, `ops.daily`). Route nothing. **Deprecate.**

### Every current event is accounted for

**21 QuickFurno Core Automation + 4 internal + 4 preview = 29 named events**, each with a disposition above. The manifest (`quickfurno-compatibility-manifest.json`) carries the same list, and a test asserts the two agree.
...
**Communication state is 18 states in the canonical model.** Core's `whatsapp-status` endpoint accepts **any** status string with no enum validation, defaulting to `"unknown"` (`QuickFurno Core AutomationSyncService.ts:256-271`) — **and updates nothing** (`databasePersisted: false`). It is a masking echo endpoint.
...
| 2   | `lib/aos/README.md:43` — _"No QuickFurno Core Automation webhook calls yet."_ | **FALSE when both locks are open** — `coreAutomationTool.ts:129` performs a real `fetch`                                                                                                                                                                                                                                 | Medium       |
...
| 4   | `queueEventForQuickFurno Core Automation` (name)                              | **There is no queue.** No persistence, no retry. A failed event is lost                                                                                                                                                                                                                                                  | Medium       |
