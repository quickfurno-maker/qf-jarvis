...
under [ADR-0136](../decisions/ADR-0136-qfj-p10-s3-fresh-quickfurno-core-audit.md), and it confirmed Model 2. **D2 — the Core protocol/event gap decision — is MERGED under [ADR-0137](../decisions/ADR-0137-qfj-p10-d2-core-protocol-and-event-gap-decision.md)**: exactly **two** Core primitive event families are targeted (`authorization-recorded`, whose future adoption path is **C3A**, still gated on C1 + C2; `result-recorded`, **not yet emission-ready**, whose path is **C3B**, gated on a Core result-contract-fit proof — **neither gate has landed**), the first durable projection is a **6-state Tier-C subset**, `completed`/`cancelled`/`expired`/`execution-submitted` are **excluded for MVP**, and the wave after it was D2a and D2b. **D2a is MERGED (PR #179, [ADR-0138](../decisions/ADR-0138-qfj-p09-d2a-accepted-event-write-path-and-provenance-hardening.md)), and D4 — the purpose-bounded trusted evidence reader — is MERGED (PR #180, [ADR-0140](../decisions/ADR-0140-qfj-p09-d4-trusted-communication-evidence-read-capability.md)). **D2b is MERGED (PR #181, [ADR-0139](../decisions/ADR-0139-qfj-p09-d2b-tier-ab-durable-evidence-and-ordering-confirmation.md)), and D3 — the first six-state `CommunicationStateRecordV2` contract — is MERGED (PR #182, [ADR-0141](../decisions/ADR-0141-qfj-p09-d3-communication-state-record-v2-six-state-contract.md)). **D5, the tiered local projection, is IMPLEMENTED OFFLINE** ([ADR-0142](../decisions/ADR-0142-qfj-p09-d5-communication-state-projection-persistence.md), Accepted; PR #183): the design was accepted at owner review and migration `0013` was separately authorized — that one alone, **repository/local/CI only, NOT applied to managed PostgreSQL**. The handler and the read model `qf_jarvis.rm_communication_state` exist and are tested against real PostgreSQL, and D4's production importer count is now **exactly 1**. **The projection is NOT in the production registry and is NOT activated.** Nothing about the Aarohi runtime changes here — it remains PLANNED / DISABLED and rollout stays OFF.** Nothing else here is
