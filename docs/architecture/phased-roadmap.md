...
| 3.4 — projections and bounded retries | **In progress. Stage 3.3.5 is merged (PR #18), so Stage 3.3 is complete. Stage 3.4.1 (projection foundation — migration `0004`, checkpoint/attempt tables, two disposable metadata read models, internal checkpoint/attempt repositories; ADR-0034) implemented locally, pending review. Stage 3.4 as a whole remains incomplete; the registry, advisory lock, runner and handlers are later slices (3.4.2–3.4.5). Migration `0004` is local/CI only; managed PostgreSQL carries only `0001`; `0002`/`0003`/`0004` unapplied; no managed readiness for `0004` claimed.** |
...
7. Stage 3.4 — projections, checkpoints, bounded retries ← Stage 3.4.1 (projection foundation, ADR-0034, migration 0004) implemented locally, pending review; registry/lock/runner/handlers are later slices; Stage 3.4 as a whole is incomplete; Stage 3.5 is not skipped
```
...
| **Stage 3.4** — projections, checkpoints, bounded retries | **Stage 3.3 (satisfied).** Stage 3.4.1 (projection foundation, ADR-0034, migration `0004`) implemented locally, pending review; later slices deliver the registry, advisory lock, runner and handlers |
