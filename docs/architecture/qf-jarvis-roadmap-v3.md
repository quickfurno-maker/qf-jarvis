...
| **QFJ-P03.08** | Rebuild Determinism and Erasure ([ADR-0043](../decisions/ADR-0043-qfj-p03-08-rebuild-determinism-and-erasure.md), design merged; implementation on a feature branch / draft PR, not merged) | Stage 3.4.5C rebuild proposal + Stage 3.6 |
...
- **QFJ-P03.08 (Rebuild Determinism and Erasure): design merged ([ADR-0043](../decisions/ADR-0043-qfj-p03-08-rebuild-determinism-and-erasure.md)); implementation on a feature branch / draft PR, not merged.** The bounded implementation adds an internal, library-only rebuild driver and canonical SHA-256 digest utility in `@qf-jarvis/event-backbone`'s projections subsystem, real-PostgreSQL determinism/erasure/isolation proofs, and the projections-scoped reducer purity lint that closes the ADR-0022 §4 gap. See the [qfj-p03-08 implementation reports](../reports/qfj-p03-08/). It uses the position-map cursor, proves live-vs-rebuild digest equality for both metadata read models, proves a synthetic erasure event's effect survives rebuild (subject-keyed erasure deferred to QFJ-P03.09), and leaves the live checkpoint/attempt/failure state unchanged. **NO_MIGRATION_REQUIRED; migration 0007 remains absent and unreserved; the package-root API remains 39 symbols; no worker/CLI/network/scheduler surface.** Merge is separately authorized after owner review.
