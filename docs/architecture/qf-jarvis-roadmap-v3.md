...
| **QFJ-P03.08** | Rebuild Determinism and Erasure ([ADR-0043](../decisions/ADR-0043-qfj-p03-08-rebuild-determinism-and-erasure.md), design documented; implementation not begun) | Stage 3.4.5C rebuild proposal + Stage 3.6 |
...
- **QFJ-P03.08 (Rebuild Determinism and Erasure): design documented, implementation not begun.** Its bounded design is fixed by [ADR-0043](../decisions/ADR-0043-qfj-p03-08-rebuild-determinism-and-erasure.md) and the [qfj-p03-08-design reports](../reports/qfj-p03-08-design/): the position-map cursor, the SHA-256 canonical-digest equivalence contract, in-place/ephemeral isolation (production shadow/cutover deferred), a synthetic erasure-survives-rebuild proof (subject-keyed erasure deferred to QFJ-P03.09), the projections purity-lint closure of the ADR-0022 §4 gap, and the bounded non-goals. **Verdict NO_MIGRATION_REQUIRED; migration 0007 remains absent and unreserved; the package-root API remains 39 symbols.** Implementation is separately authorized.
