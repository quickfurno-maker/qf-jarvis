...
| 3.4 | Projections, checkpoints, bounded retries | **In progress. Stage 3.4.1 (projection foundation — migration `0004`, `projection_checkpoint`/`projection_attempt` + two disposable metadata read models, internal repositories; ADR-0034) implemented locally, pending review. The two models are `rm_event_type_activity` and `rm_daily_event_acceptance`; `rm_subject_activity` stays deferred to Stage 3.6. Registry/lock/runner/handlers are later slices; the full-stage gate remains: poison contained, other projections unaffected. `0004` is local/CI only; managed carries only `0001`; `0002`/`0003`/`0004` all pending; **no managed run is authorized** — the default migrator applies all pending migrations, so a run would apply `0002`, `0003` and `0004`.** |
