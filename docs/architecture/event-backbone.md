...
The earlier `RetryableError` / `TerminalError` two-class taxonomy was **design vocabulary and is not implemented**: the runner classifies a handler failure by inspecting the caught value's SQLSTATE as a closed **tri-state** — **absent** (no own `code`), a **valid** well-formed code decided by the handler-side/infrastructure tables, or an **unreadable** code (an accessor, a non-string, or a malformed value) which is conservative infrastructure ([ADR-0037](../decisions/ADR-0037-stage-3-4-4-projection-runner.md) §6, superseding ADR-0021 §2's handler-error taxonomy and its "5 (configurable)" annotation — the number five is preserved exactly). **QFJ-P03.07B correction ([ADR-0040](../decisions/ADR-0040-projection-failure-operations-quarantine-and-authorized-replay.md), [projection-failure-operations.md](./projection-failure-operations.md)):** the classifier is now a closed five-category taxonomy (`DETERMINISTIC_HANDLER_FAILURE`, `TRANSIENT_INFRASTRUCTURE_FAILURE`, `REPOSITORY_INVARIANT_FAILURE`, `CANCELLATION_OR_SHUTDOWN`, `UNKNOWN_UNCLASSIFIED_FAILURE`) in `projection-failure-taxonomy.ts`. An **absent** code was deterministic through QFJ-P03.06 and is **now `UNKNOWN_UNCLASSIFIED_FAILURE`** — it fails closed, records **no** attempt, and never blocks; a deterministic failure now requires the explicit repository-owned deterministic-handler-failure contract **or** a recognised deterministic SQLSTATE. The retry/ordering/checkpoint invariants and the package-root 39-symbol surface are unchanged. **QFJ-P03.07C persistence foundation ([ADR-0040](../decisions/ADR-0040-projection-failure-operations-quarantine-and-authorized-replay.md), [migration-ledger.md](../governance/migration-ledger.md)):** migration `0006_projection_failure_operations.sql` adds the durable failure-persistence tables — `projection_failure` (aggregate), the append-only `projection_failure_action` ledger, `projection_replay_authorization`, and `projection_replay_attempt` — with immutability/transition triggers, partial-unique active-failure/active-authorization/live-attempt indexes, a composite authorization↔failure FK, least-privilege grants, and repository contracts plus a fail-closed divergence detector. **QFJ-P03.07D atomic retry-exhaustion integration is now implemented:** the runner's **fifth deterministic failure** atomically establishes the durable failure aggregate and its append-only `created` action in the SAME READ COMMITTED transaction as the final attempt and the checkpoint block — the final attempt, the blocked checkpoint, the blocked position, **exactly one** active failure aggregate, and the creation action commit or roll back together — and a later invocation that finds an already-blocked checkpoint proves the active-failure biconditional (exactly one active failure, at the blocked position) before returning `blocked-existing`, failing closed on a divergence and never repairing. Only the deterministic-exhaustion path creates an aggregate: an infrastructure, repository-invariant, cancellation, or unknown failure still rolls back and creates none. **QFJ-P03.07E failure inspection and quarantine operations are now implemented:** an internal operations boundary (`projection-failure-operations.ts`) provides authorized, bounded, keyset-paginated failure inspection, action-history and divergence inspection, and the two non-replay lifecycle mutations — **acknowledge** (`open → acknowledged`) and **quarantine** (`open`/`acknowledged → quarantined`). Every mutation is authorized through an injected authorizer (closed `projection-failure:inspect`/`:acknowledge`/`:quarantine` capabilities, quarantine strictly stronger than inspect), generation-guarded, idempotent (deterministic action-ledger idempotency key), atomic (status transition + exactly one append-only action in one transaction), and — for quarantine — gated on the fail-closed divergence detector. **Quarantine is failure-lifecycle metadata, not projection progress: it preserves the blocked checkpoint, the blocked position, `last_position`, every attempt, the failure identity, and the action history — it never advances or resets the checkpoint, never skips the event, never resolves the failure, and never creates or executes a replay.** **QFJ-P03.07F authorized projection replay is now implemented:** an internal replay service (`projection-failure-replay.ts`) provides the explicit, human-authorized, lease-protected, idempotent, crash-recoverable, **one-shot** replay path for a quarantined failure — `authorize` (quarantined → replay-authorized, one active authorization), `execute` (claim a lease-protected `started` attempt → replaying → apply the registered handler to the exact blocked event under the name+version advisory lock), atomic `success`, deterministic-failure recording, expired-lease `takeover`, and read-only `reconcile`. **Replay success is atomic:** in one transaction it applies the read-model mutation, resumes the checkpoint exactly to the blocked position (the only transition out of `blocked`, via `resumeCheckpointAfterReplay`), succeeds the attempt, resolves the failure, consumes the authorization, and appends append-only evidence. A replay **never** advances the checkpoint on failure (a deterministic failure returns the failure to quarantined, the checkpoint stays blocked), never skips, and never happens without an explicit authorization. **There is NO automatic replay, scheduler, poller, timer, background loop, or bulk replay** — every operation is explicitly invoked with the exact authorized failure, and an ambiguous COMMIT is reconciled before any retry. **Migration 0006 is unchanged and applied local/CI only (not managed, not deployed); migration 0007 does not exist; and the package-root API stays at 39. QFJ-P03.07G (observability, runbook, exit audit) remains next.** The inspected caught value is **property-bearing** — an **object or a function** (a thrown function can still carry an own `code`); only a true primitive is "no code". A thrown handler value is always **discarded**, never wrapped as `cause`, and **never allowed to dictate its own classification**: an accessor `code` is treated as unreadable rather than "no code", its getter is never invoked, and the descriptor inspection is guarded so a **revoked object or function `Proxy`** cannot leak a raw exception. Beyond classification, the outer transaction's lifecycle is tracked so the client is **released normally only after a confirmed COMMIT or a confirmed outer ROLLBACK** and destroyed otherwise. If anything unexpected escapes with the transaction still open, a final safety boundary attempts an outer ROLLBACK: a **successful** rollback leaves the state `closed` and the client gets a **normal, reusable release**; only a **failed** rollback leaves the state `unknown` and destroys the client via `release(true)` — and the caught value and any cleanup error are never preserved or leaked. Even recognising the runner's own error type at that boundary is **guarded**, because a bare `instanceof` on a revoked `Proxy` would itself throw.
