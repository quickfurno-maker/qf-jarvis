...
| 3.3 | Ingestion, dedup, conflict detection | **Slices done: atomic idempotent persistence (`storeValidatedEvent`, slice 3) and the append-only `ingestion_rejection` + `event_conflict` audit tables with in-transaction conflict recording (Stage 3.3.3, migration `0003`, ADR-0031). Idempotency, conflict, concurrency, append-only, and column-level runtime privilege proven against local PostgreSQL. Implemented and verified locally; pending owner review; migrations `0002` and `0003` not applied to the managed database (which carries `0001`). Full `ingest`/worker (Stage 3.3.4) outstanding** |
...
**Stage 3.1 is reviewed and merged; Stage 3.2 (pure signature verification) is complete, owner-accepted and merged (PR #10, 2026-07-16). Managed-database readiness is established and migration `0001_event_log.sql` is applied. Stage 3.3 slice 3 (atomic, idempotent persistence) and Stage 3.3.3 (the append-only `ingestion_rejection` and `event_conflict` tables, migration `0003`, ADR-0031) are implemented and verified locally; pending owner review; migrations `0002_event_runtime_grants.sql` and `0003_ingestion_rejection_and_event_conflict.sql` are not applied to the managed database. Stage 3.3.4 (the full ingest composition), Stage 3.3 as a whole, and Phase 3 are NOT complete.** Approval of the *decisions* is not authorisation to *implement* them, and approval of one stage is not authorisation for the next.
