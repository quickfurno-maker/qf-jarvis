/**
 * JAO-5 integration-test plumbing, and the guards that stop it ever running anywhere real.
 *
 * Not a test file. This is the ONLY module in the JAO-5 slice that reads `DATABASE_URL` or touches
 ...
 *
 * Without `DATABASE_URL` every JAO-5 database test FAILS. Durability across a restart is the entire
 ...
 *
 * `qf_jarvis_jao5`, created by the local JAO-5 schema asset. It never touches `public`, never
 * touches `qf_jarvis` -- the managed event-backbone schema -- and never runs a managed migration.
 * That separation is the point: JAO-5's schema is a local asset for this proof, not an entry in
...
export const JAO5_TEST_SCHEMA = 'qf_jarvis_jao5';

export function requireJao5TestDatabaseUrl(): string {
  const url = process.env['DATABASE_URL'];
  if (url === undefined || url.trim() === '') {
    throw new Error(
      [
        'DATABASE_URL is not set, so the JAO-5 ambient governance tests cannot run.',
        '',
        'These tests FAIL rather than skip. JAO-5 claims that operational memory survives a',
...
export function jao5TestDatabaseConfig(applicationName: string): DatabaseConfig {
  return createDatabaseConfig({
    connectionString: requireJao5TestDatabaseUrl(),
 ...
 */
export function createJao5TestPool(applicationName: string): DatabasePool {
  return createDatabasePool(jao5TestDatabaseConfig(applicationName));
}

/** The local schema asset, read from source. Deliberately not a managed migration. */
export function readJao5SchemaSql(): string {
  return readFileSync(
    fileURLToPath(
      new URL(
        '../jao/controlled-ambient-operations/schema/001_jao5_ambient_monitor_state.sql',
        import.meta.url,
      ),
    ),
    'utf8',
...
/**
 * Drop JAO-5's own schema and apply the local asset, so a suite starts from nothing.
 *
 ...
 */
export async function resetJao5Schema(pool: DatabasePool): Promise<void> {
  requireJao5TestDatabaseUrl();
  await withClient(pool, async (client) => {
    await client.query(`DROP SCHEMA IF EXISTS ${JAO5_TEST_SCHEMA} CASCADE`);
    await client.query(readJao5SchemaSql());
  });
}

/** Apply the schema without dropping, proving the asset is safely re-appliable. */
export async function applyJao5Schema(pool: DatabasePool): Promise<void> {
  await withClient(pool, async (client) => {
    await client.query(readJao5SchemaSql());
  });
}
...
export async function countJao5Rows(pool: DatabasePool, table: string): Promise<number> {
  return withClient(pool, async (client) => {
    const result = await client.query<{ readonly total: string }>(
      `SELECT count(*)::text AS total FROM ${JAO5_TEST_SCHEMA}.${table}`,
    );
...
export async function countJao5RowsFor(
  ...
  return withClient(pool, async (client) => {
    const result = await client.query<{ readonly total: string }>(
      `SELECT count(*)::text AS total FROM ${JAO5_TEST_SCHEMA}.${table} WHERE monitor_instance_id = $1`,
      [monitorInstanceId],
    );
    return Number(result.rows[0]?.total ?? '0');
  });
...
export async function forceJao5RawUpdate(
  pool: DatabasePool,
  sql: string,
