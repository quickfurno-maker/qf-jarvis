 ...
 *
 * The Groq connectivity check is the existing staging smoke. JF-5B-R25 removes every Nara
 * credential/discovery seam from this production composition; the current release is GROQ_ONLY with no
 * hosted fallback. The agent turns still run through the existing three-agent Mastra composition and
 * the QF Model Gateway. There is no second workflow or alternate provider path here.
 *
