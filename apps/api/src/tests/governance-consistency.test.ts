...
describe('QFJ-P12 (ADR-0085) canonical governance consistency', () => {
  ...
  describe('canonical status lines do not invalidate themselves (JOS-01B, ADR-0086)', () => {
    ...
    it('records the main track resuming at QFJ-P09.02 and continuing at QFJ-P09.03', () => {
      const roadmap = flat(read(ROADMAP));
      // The JOS foundation track closed with JOS-01E and the main track picked up at P09.02, which
      // has since merged. The resume point is a historical fact and stays; what must move is where
      // the track is NOW.
      expect(roadmap).toMatch(/Jarvis OS foundation track is CLOSED/u);
      expect(roadmap).toMatch(/resumed at QFJ-P09\.02/u);
      expect(roadmap).toMatch(/continues at QFJ-P09\.03/u);
